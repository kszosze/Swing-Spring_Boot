...
public class EditSalesOrder extends EditContentPanel {
	...
	public boolean bindToGUI(Object o) {
		...
		if (o != null) {
			...
			txtOrderNum.setText(salesOrder.getOrderNumber().toString());
			txtCustomer
					.addItem(new ComboBoxItem(salesOrder.getCustomer().getCode(), salesOrder.getCustomer().getName()));
			defaultTableModel.setRowCount(0);
			for (OrderLine orderLine : salesOrder.getOrderLines()) {
				defaultTableModel.addRow(orderLine.toRowModel());
			}

			txtTotalPrice.setText(salesOrder.getTotal().toString());
			return true;
		} else {
			return false;
		}
	...
	public Object guiToObject() {
		...
		salesOrder.setOrderNumber(Long.valueOf(txtOrderNum.getText()));
		Customer customer = (Customer) Services
				.readRecordByCode(((ComboBoxItem) txtCustomer.getSelectedItem()).getKey(), Services.TYPE_CUSTOMER);
		salesOrder.setCustomer(customer);
		for (int i = 0; i < defaultTableModel.getRowCount(); i++) {
			OrderLine orderLine = new OrderLine();
			orderLine.setSalesOrder(salesOrder);
			Product product = (Product) Services.readRecordByCode((String) defaultTableModel.getValueAt(i, 0),
					Services.TYPE_PRODUCT);
			orderLine.setProduct(product);
			orderLine.setQuantity(Integer.valueOf((String) defaultTableModel.getValueAt(i, 1)));
			salesOrder.getOrderLines().add(orderLine);
		}
		salesOrder.setTotal(Float.valueOf(txtTotalPrice.getText()));
		return salesOrder;
	}
